...
$sudo apt install cmake zenity libstdc++6 mesa* libasound* build-essential -y
...
case $input in
	1)
unset version
version=`curl https://mirrors.tuna.tsinghua.edu.cn/winehq/wine/wine-mono/ | grep href | tail -1 | awk -F 'href="' '{print $2}' | cut -d '/' -f 1`
wget https://mirrors.tuna.tsinghua.edu.cn/winehq/wine/wine-mono/$version/wine-mono-$version-x86.msi ;;

	2)
unset version
version=`curl https://mirrors.tuna.tsinghua.edu.cn/winehq/wine/wine-gecko/ | grep href | tail -1 | awk -F 'href="' '{print $2}' | cut -d '/' -f 1`
wget https://mirrors.tuna.tsinghua.edu.cn/winehq/wine/wine-gecko/$version/wine-gecko-$version-x86.msi 
unset version
version=`curl https://mirrors.tuna.tsinghua.edu.cn/winehq/wine/wine-gecko/ | grep href | tail -1 | awk -F 'href="' '{print $2}' | cut -d '/' -f 1`
wget https://mirrors.tuna.tsinghua.edu.cn/winehq/wine/wine-gecko/$version/wine-gecko-$version-x86_64.msi ;;
	*) echo "" ;;
esac
main
}
...
mkdir bullsbox
tar xvf rootfs.tar.xz -C bullsbox
rm rootfs.tar.xz
echo -e "\e[33m系统已下载,文件夹名为bullsbox\e[0m"
sleep 2
echo $(uname -a) | sed 's/Android/GNU\/Linux/' >bullsbox/proc/version
if [ ! -f "bullsbox/usr/bin/perl" ]; then
        cp bullsbox/usr/bin/perl* bullsbox/usr/bin/perl
fi
sed -i "3i\rm -rf \/tmp\/.X\*" bullsbox/etc/profile
sed -i "/zh_CN.UTF/s/#//" bullsbox/etc/locale.gen
rm bullsbox/etc/resolv.conf 2>/dev/null
echo "nameserver 223.5.5.5
nameserver 223.6.6.6" >bullsbox/etc/resolv.conf
...
#deb http://mirrors.bfsu.edu.cn/debian-security/ bullseye/updates main contrib non-free
deb http://mirrors.bfsu.edu.cn/debian-security bullseye-security main contrib non-free' >bullsbox/etc/apt/sources.list
echo ". firstrun" >>bullsbox/etc/profile
cat >bullsbox/root/firstrun<<-'eof'
...
pulseaudio --start &
unset LD_PRELOAD
proot --kill-on-exit -S bullsbox --link2symlink -b /sdcard:/root/sdcard -b /sdcard -b bullsbox/proc/version:/proc/version -b bullsbox/root:/dev/shm -w /root /usr/bin/env -i HOME=/root TERM=$TERM USER=root PATH=/usr/local/sbin:/usr/local/bin:/bin:/usr/bin:/sbin:/usr/sbin:/usr/games:/usr/local/games LANG=C.UTF-8 TZ=Asia/Shanghai /bin/bash --login" >start-bullsbox.sh && chmod +x start-bullsbox.sh
echo -e "已创建root用户系统登录脚本,登录方式为\e[33m./start-bullsbox.sh\e[0m"
}
main(){                                                       clear
echo -e "
