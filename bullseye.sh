...
sleep 2
if [ -d ${HOME}/.t_bullseye ]; then echo -e "\n检测已有相关文件夹，是否删除？\n\e[33m1)删除重装\n2)删除\n0)退出\e[0m\n"
	read -r -p "请选择: " input
	case $input in
		1) rm -rf ${HOME}/.t_bullseye ;;
		2) rm -rvf ${HOME}/.t_bullseye ${PREFIX}/bin/start-bullseye /data/data/com.termux/files0 ${PREFIX}/bin/start-androiduser && exit 0 ;;
		*) exit 0
	esac
fi
...
;;
*)
curl -O ${URL}/lxc-images/images/debian/bullseye/arm64/default/$(curl ${URL}/lxc-images/images/debian/bullseye/arm64/default/ | grep href | tail -n 2 | cut -d '"' -f 4 | head -n 1)rootfs.tar.xz
if [ ! -f rootfs.tar.xz ]; then echo -e "\e[31m下载错误，请检查网络\e[0m"; sleep 1; exit 0; fi
mkdir ${HOME}/.t_bullseye
tar xvf rootfs.tar.xz -C ${HOME}/.t_bullseye
rm rootfs.tar.xz
esac

#for i in boot dev etc mnt opt proc run sys usr var; do if [[ ! -d ${HOME}/.t_bullseye/$i ]]; then echo -e "\e[31m下载错误，请检查网络\e[0m"; sleep 1; exit 0; fi done

curl -o ${HOME}/.t_bullseye/root/openssl.deb ${URL}/debian/pool/main/o/openssl/$(curl ${URL}/debian/pool/main/o/openssl/|grep openssl_1.*arm64.deb|awk -F 'href="' '{print $2}'|cut -d '"' -f 1|tail -n 1)
curl -o ${HOME}/.t_bullseye/root/ca-certificates.deb ${URL}/debian/pool/main/c/ca-certificates/$(curl ${URL}/debian/pool/main/c/ca-certificates/|grep all.deb|awk -F 'href="' '{print $2}'|cut -d '"' -f 1|tail -n 1)
mkdir -p /data/data/com.termux/files0/home
echo -e "\e[33m系统已下载,文件夹名为.t_bullseye\e[0m"
sleep 2

#伪proc文件
PROC_DIR=".t_bullseye/etc/proc"
...
#echo "service dbus start" >>${HOME}/.t_bullseye/root/.bashrc
...
#添加termux用户
echo "$(id -un):x:$(id -u):$(id -g):Android user:/data/data/com.termux/files0/home:/bin/bash" >> "${HOME}/.t_bullseye/etc/passwd"
echo "$(id -un):*:18446:0:99999:7:::" >> "${HOME}/.t_bullseye/etc/shadow"
while read -r group_name group_id; do
echo "${group_name}:x:${group_id}:root,$(id -un)" >> ${HOME}/${ETC_DIR}/group
if [ -f ".t_bullseye/etc/gshadow" ]; then
echo "${group_name}:*::root,$(id -un)" >> ${HOME}/${ETC_DIR}/gshadow
fi
done < <(paste <(id -Gn | tr ' ' '\n') <(id -G | tr ' ' '\n'))
cat >${HOME}/.t_bullseye/root/firstrun<<-'eof'
#!/usr/bin/env bash
export LANG=C.UTF-8
cd
...
DEPENDS="apt-utils sudo busybox locales curl elementary-icon-theme wget vim fonts-wqy-microhei tar lxtask dbus-x11 tigervnc-standalone-server tigervnc-viewer libtinfo5 ristretto python3 pulseaudio xserver-xorg x11-utils minetest psmisc procps"
...
ln -sf /usr/games/minetest /usr/local/bin
...
mkdir ${HOME}/.minetest 2>/dev/null
echo 'language = en_US' >>${HOME}/.minetest/minetest.conf
...
登录容器请输\e[33mstart-bullseye\e[0m
登录容器普通用户请输\e[33mstart-androiduser\e[0m
打开vnc请输\e[33measyvnc\e[0m\nvnc viewer地址输127.0.0.1:0，免密登录。
容器的退出,在系统输exit即可
删除容器请输\e[33mstart-bullseye --purge\e[0m"
...
echo 'printf "\n你当前使用的系统是 $(cat /etc/os-release | sed -n 1p | cut -d "=" -f 2)\n"' >>${HOME}/.t_bullseye/root/.bashrc

cat >${PREFIX}/bin/start-bullseye<<eof
#!/usr/bin/env bash
cd
case \$1 in
	...
	read -r -p "" input
	case \$input in
		Y|y) rm -rfv ${PREFIX}/bin/start-bullseye /data/data/com.termux/files0 ${PREFIX}/bin/start-androiduser
		rm -rf .t_bullseye
			if [ -d .t_bullseye ]; then
			echo -e "删除失败"
		else
		echo -e "已删除"
			fi
...
FIRSTRUN=".t_bullseye/root/firstrun"
...
;;
	*)
sed -i '2i WM="xfce4 xfce4-terminal xfce4-weather-plugin"' ${HOME}/.t_bullseye/root/firstrun
esac
echo -e "已创建root用户系统登录脚本,登录方式为\e[33mstart-bullseye\e[0m\n"
sleep 2
